...
% The critical transition in \eqref{equ:Example:GenActivation:1D} is determined by a critical saddle-node bifurcation where the stable equilibrium in which gen activity is low collides with an unstable equilibrium, leading trajectories towards the remaining stable equilibrium in which gen activity is high. If $s$ is transformed into a variable, we would expect a similar critical saddle-node bifurcation to occur but in a higher dimension.
